 ...
 */
UCLASS(abstract)
class TEMPORALDASH_API AShooterWeapon : public AActor
{
	GENERATED_BODY()
...
protected:
	...
	// The maximum number of shots before the weapon is discarded.
	// Make this Blueprint-editable so you can set it per weapon.
	UPROPERTY(EditDefaultsOnly, Category = "Weapon|Ammo", meta=(ClampMin=1, ClampMax=100))
	int32 MaxMagazines = 1; // default 1 magazine (no spare)

	/** Remaining spare magazines NOT currently loaded (decrements only when we reload) */
	UPROPERTY(VisibleAnywhere, Category = "Weapon|Ammo")
	int32 RemainingMagazines = 0;

	/** If true, weapon updates HUD every shot (disable for high ROF weapons to reduce UI overhead) */
	UPROPERTY(EditAnywhere, Category="HUD")
	bool bUpdateHUDPerShot = true;
...
public:
	...
	/** Returns true if the weapon has no bullets and no spare magazines */
	bool IsEmpty() const { return CurrentBullets <= 0 && RemainingMagazines <= 0; }

	/** Handle discard logic (deprecated: use IsEmpty + owner DiscardWeapon) */
	void HandleEmpty();
...
public:
	...
	// Optionally expose remaining magazines for UI if needed later
